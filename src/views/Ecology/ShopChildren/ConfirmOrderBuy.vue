  ...
  <div id="confirm">
    ...
    <div class="order-detail" v-if="address" @click="chooseAddress()">
      <div class="order-list">
        <img style="width: 0.666666rem;height: 0.8rem;" src="~assets/img/shop/icon_dizhi02@2x.png" alt="">
        <div style="flex: 1">
          <p>{{goChooseAddress.user_name || address.user_name}}<span class="order-time">{{goChooseAddress.user_phone || address.user_phone}}</span></p>
    ...
    <div class="shop-detail">
      <div class="shop-list">
        <img :src="goodsInfo.cover_image | imgUrl" alt="">
        <div class="shop-list-detail" style="width:6.6rem">
          <p class="shop-head">{{goodsInfo.title}}</p>
          <p class="shop-size">{{goodsInfo.specifications}}</p>
          <div class="shop-money-div">
            <p class="shop-money">{{goodsInfo.price}}FND</p>
      ...
      <div class="size-list">
        ...
        <div class="buy-number-edit">
          <p style="color: #acacac;" @click="removeNumber">-</p>
          <p class="change-number">{{formData.number}}</p>
          <p style="color: #acacac;" @click="addNumber">+</p>
        </div>
      </div>
      <div class="size-list">
        <p class="buy-number">订单备注</p>
        <textarea class="textarea" v-model="formData.note" placeholder="选填，请先和商家协商一致"></textarea>
      </div>
      <p style="margin-top: 0.266666rem;padding-right: 0.266666rem;text-align: right;font-size: 0.32rem;">
        <span style="color: #ACACAC;font-size: 0.32rem;" class="sum">共{{formData.number}}件</span>
        小计:
        <span style="color: #f33;font-size: 0.32rem;" class="totalPrice">￥{{Number(formData.number * goodsInfo.price).toFixed(4)}}</span>
      </p>
    </div>
    <div class="confirm-bottom">
      <span style="padding-right: 0.266666rem;color: #ACACAC;font-size: 0.32rem;" class="sum">共{{formData.number}}件</span>
      <span style="font-size: 0.32rem;color: #282828;">合计:</span>
      <span style="padding-right: 0.266666rem;color: #f33;font-size: 0.4rem;" class="totalPrice">{{Number(formData.number * goodsInfo.price).toFixed(4)}}FND</span>
    ...
    <div class="confirm_pay_tan confirm_pay_model" v-show="confirmPay">
      <div class="confirm_pay">
        ...
        <h2>{{formData.number * goodsInfo.price}}<span>FND</span></h2>
        <!-- <h5>付款金额:
          <p class="choose-pay-type">{{formData.number * goodsInfo.price}}</p>
        </h5> -->
    ...
    <van-popup v-model="isShowPay">
      <div class="van_confirm_pay">
        ...
        <h2 id="totalPrice">{{formData.number * goodsInfo.price}}<span style="font-size: 0.28rem;">FND</span></h2>
        <div>
...
<script>
import {confirmorder,ordersubmit} from '@/request/api'
...
export default {
  ...
  methods: {
    ...
    chooseAddress() {
      localStorage.setItem('confirmOrder','/shopChildren/confirmOrderBuy'),
      this.$router.push('/shopChildren/shopAddress')
    },
    confirmorder() {
      const _this = this
      confirmorder({goods_id: this.$route.query.goods_id,number: this.$route.query.number})
        .then((result) => {
          _this.address = result.data.address
          _this.goodsInfo = result.data.goodsInfo
          _this.formData.number = result.data.number
        })
    },
    locationBack() {
      window.history.go(-1)
    ...
    removeNumber() {
      this.formData.number == 1 ? this.formData.number : this.formData.number--
    },
    orderClick() {
  ...
  created() {
    ...
    if(localStorage.getItem('confirmOrder')){
      this.goChooseAddress = localStorage.getItem('chooseAddress') ? JSON.parse(localStorage.getItem('chooseAddress')) : {}
